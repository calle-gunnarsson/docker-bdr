...
if [ "$1" = 'postgres' ]; then
	...
	if [ ! -s "$PGDATA/PG_VERSION" ]; then
		...
		echo "host replication postgres samenet trust";
		} >> "$PGDATA/pg_hba.conf"

		{ echo;
		echo "max_worker_processes = 11"
		echo "max_replication_slots=10"
		echo "max_wal_senders=10"
		echo "wal_level = 'logical'"
		echo "shared_preload_libraries='uuid-ossp,bdr'"
		...
		if [ "$POSTGRES_DB" != 'postgres' ]; then
			...
			EOSQL
			"${psql[@]}" --username postgres $POSTGRES_DB <<-EOSQL
